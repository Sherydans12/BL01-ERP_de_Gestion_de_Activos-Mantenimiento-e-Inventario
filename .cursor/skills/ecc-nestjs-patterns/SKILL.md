...
description: >-
  Patrones NestJS (módulos, DTOs, guards, pipes, transacciones, tests) para el backend TPM.
  Complementa tpm-arquitectura.mdc; no sustituye tenantId, siteId ni convenciones Prisma del proyecto.
---

# Patrones NestJS (BL01 + referencia ECC)

## Prioridad en este repositorio

1. Reglas siempre activas: **`.cursor/rules/tpm-arquitectura.mdc`**.
2. Este skill aporta patrones generales de NestJS donde no contradiga el punto anterior.
3. Estructura real del backend: **`backend/src/features/<dominio>/`** y **`backend/src/common/`**. El diagrama `modules/` más abajo es plantilla genérica (everything-claude-code).
...
_Origen: skill `nestjs-patterns` del repo [everything-claude-code](https://github.com/affaan-m/everything-claude-code) (adaptada a BL01)._
