...
| `INVENTORY_ITEM_CREATED` | `Nuevo artículo en catálogo: {código} — {nombre}` | Alta de artículo en catálogo maestro | ccEmails del tenant (solo correo externo; userIds vacío) | `inventory-items.service` → `create` | `buildMailInventoryItemCreated` | — | **Dispatcher** |
